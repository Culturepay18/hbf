...
import { Plus, Edit2, Trash2, Calendar, MapPin, Trophy, Link as LinkIcon, Save, X, Loader2 } from "lucide-react";
    ...
    <div className="admin-scholarships-page">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="mt-1 text-hbf-muted">Manage scholarship programs, prizes, and application links.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-black/10 bg-white py-24 text-center">
          ...
          <p className="mt-2 text-hbf-muted">Add your first scholarship program to the platform.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 mx-4 w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between border-b border-black/5 px-8 py-5">
              <h3 className="text-xl font-bold text-hbf-dark">
                {editingId ? "Edit Scholarship" : "New Scholarship"}
              </h3>
              <button onClick={closeModal} className="p-2 hover:bg-black/5 rounded-full text-hbf-muted">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea rows={3} value={formData.description || ""} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className={`${inp} resize-none`} placeholder="Short summary of the program..." />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={formData.location || ""} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className={inp} placeholder="Ex: Cap-Haitien" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={lbl}>Prize / Support</label>
                  <input type="text" value={formData.prize || ""} onChange={(e) => setFormData({ ...formData, prize: e.target.value })} className={inp} placeholder="Ex: 5 MacBooks + $1,000" />
                </div>
                <div>
              ...
              <div>
                <label className={lbl}>Details Link (External URL)</label>
                <input type="text" value={formData.details_link || ""} onChange={(e) => setFormData({ ...formData, details_link: e.target.value })} className={inp} placeholder="https://..." />
              </div>
            </div>

            <div className="flex items-center justify-end gap-3 border-t border-black/5 bg-[#f8f6f0]/30 px-8 py-5">
              <button onClick={closeModal} className="px-5 py-2 text-sm font-bold text-hbf-muted hover:text-hbf-dark">
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="flex items-center gap-2 rounded-xl bg-hbf-green px-8 py-3 text-sm font-bold text-white shadow-lg transition hover:bg-hbf-green-light disabled:opacity-50"
              >
